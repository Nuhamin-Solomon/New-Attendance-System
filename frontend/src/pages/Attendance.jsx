...
export default function Attendance() {
  ...
  const filtered = useMemo(() => attendance.filter((row) => matchesSearch(row, query, ["full_name", "card_id", "department", "source"])), [attendance, query]);
  return <section className="fade-in">
    ...
    <div className="panel"><div className="panel-header"><div><div className="panel-title">Attendance log</div><div className="panel-subtitle">Most recent scans appear first</div></div><label className="search-bar"><Icon name="search" size={16}/><input aria-label="Search attendance" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by name, ID, or department" /></label></div>
